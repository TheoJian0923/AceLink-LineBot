    ...
    try
    {
        ...
        foreach (var ev in json.events)
        {
            ...
            if(userMessage == "免費立即試用")
            {
                ...
                tryMsg.AppendLine("");
                tryMsg.AppendLine("感謝您對 AceLink 有興趣！");
                tryMsg.AppendLine("為了讓您有更好的試用體驗，請先留下基本聯絡資料。");
                tryMsg.AppendLine("");
                tryMsg.AppendLine("負責人將於一天內與您聯繫，並協助您進行功能介紹、試用說明與初始設定。");
                tryMsg.AppendLine("");
                tryMsg.AppendLine("━━━━━━━━━━━━━━");
                tryMsg.AppendLine("📌 請先提供以下資料");
                tryMsg.AppendLine("");
                tryMsg.AppendLine("1️⃣ 聯絡人稱呼");
                tryMsg.AppendLine("2️⃣ 聯絡方式：LINE ID / 手機");
                tryMsg.AppendLine("3️⃣ 球館名稱");
                tryMsg.AppendLine("4️⃣ 得知管道");
                tryMsg.AppendLine("5️⃣ 目前管理方式：手動統計 / Google 表單 / Excel / 其他");
                tryMsg.AppendLine("");
                tryMsg.AppendLine("━━━━━━━━━━━━━━");
                tryMsg.AppendLine("📋 可直接複製以下格式回覆");
                tryMsg.AppendLine("");
                ...
                tryMsg.AppendLine("");
                tryMsg.AppendLine("收到後，我們會盡快與您聯繫，協助您了解 AceLink 是否適合目前的球團管理方式。");
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (action == "+")
                            {
                                ...
                                //if (!isAdmin && !data.IsWithinRegistrationPeriod(out string formattedRange))//測試用，會阻擋開發者
